...
(function () {
    ...
    const I18N = {
        'zh-CN': {
            ...
            welcome_tip: '支持的文件格式：.md、.markdown、.txt、.html',
            ...
            batch_move_success: '成功移动 {count} 项', batch_move_fail: '批量移动失败',
            batch_move_confirm: '确定要将 {count} 项移动到 "{target}" 吗？',
            batch_delete_confirm: '确定要删除选中的 {count} 项吗？此操作不可恢复。',
            batch_delete_success: '成功删除 {count} 项', batch_delete_fail: '批量删除失败',
            delete_selected: '删除选中文档', delete_selected_title: '删除选中文档（{count} 个）',
            invalid_drop_target: '只能移动到文件夹中',
            rename_success: '重命名成功',
            upload_success: '成功上传 {count} 个文件',
            upload_fail_count: '，失败 {count} 个',
            upload_fail: '上传失败',
            no_supported_files: '没有找到支持的文件（.md, .markdown, .txt, .html）',
            ...
            ai_progress_title: 'AI 操作过程',
            ai_progress_done: '操作完成',
            reasoning_title: '深度思考',
        ...
        'en': {
            ...
            welcome_tip: 'Supported formats: .md, .markdown, .txt, .html',
            ...
            batch_move_success: 'Moved {count} item(s) successfully', batch_move_fail: 'Batch move failed',
            batch_move_confirm: 'Move {count} item(s) to "{target}"?',
            batch_delete_confirm: 'Delete {count} selected item(s)? This cannot be undone.',
            batch_delete_success: 'Deleted {count} item(s) successfully', batch_delete_fail: 'Batch delete failed',
            delete_selected: 'Delete selected', delete_selected_title: 'Delete selected ({count})',
            invalid_drop_target: 'Can only drop onto folders',
            rename_success: 'Renamed successfully',
            upload_success: 'Successfully uploaded {count} file(s)',
            upload_fail_count: ', {count} failed',
            upload_fail: 'Upload failed',
            no_supported_files: 'No supported files found (.md, .markdown, .txt, .html)',
            ...
            ai_progress_title: 'AI Progress',
            ai_progress_done: 'Completed',
            reasoning_title: 'Deep Thinking',
        ...
        'fr': {
            ...
            welcome_tip: 'Formats supportés : .md, .markdown, .txt, .html',
            ...
            batch_move_success: '{count} élément(s) déplacé(s)', batch_move_fail: 'Échec du déplacement groupé',
            batch_move_confirm: 'Déplacer {count} élément(s) vers « {target} » ?',
            invalid_drop_target: 'Déposer uniquement sur des dossiers',
            ...
            no_supported_files: 'Aucun fichier supporté (.md, .markdown, .txt, .html)',
            ...
            ai_progress_title: 'Progression IA',
            ai_progress_done: 'Terminé',
            reasoning_title: 'Réflexion approfondie',
        ...
        'ja': {
            ...
            welcome_tip: '対応形式：.md、.markdown、.txt、.html',
            ...
            batch_move_success: '{count}件移動しました', batch_move_fail: '一括移動に失敗しました',
            batch_move_confirm: '{count}件を「{target}」に移動しますか？',
            invalid_drop_target: 'フォルダにのみドロップできます',
            ...
            ai_progress_title: 'AIの進行状況',
            ai_progress_done: '完了',
            reasoning_title: '深い思考',
    ...
    function applyLanguage() {
        ...
        _a('#sidebarCollapseBtn', 'title', 'collapse_sidebar');
        if (typeof updateToggleAllSelectBtn === 'function') updateToggleAllSelectBtn();
        ...
        // Edit button text
        const editSpan = document.querySelector('#toggleEditBtn .btn-text');
        if (editSpan) editSpan.textContent = (typeof isEditingPreview !== 'undefined' && isEditingPreview) ? t('preview_mode') : t('edit');
        _a('#toggleEditBtn', 'title', 'edit_document');

        // Editor toolbar
        const editorLabel = document.querySelector('.preview-editor-label');
        if (editorLabel) editorLabel.textContent = t('edit_document');
        const editorHint = document.querySelector('.preview-editor-hint');
        if (editorHint) editorHint.textContent = 'Ctrl+S ' + t('save');
        const saveBtnSpan = document.querySelector('#editorSaveBtn .btn-text');
        ...
        const tocSpan = document.querySelector('#tocToggleBtn .btn-text');
        if (tocSpan) tocSpan.textContent = t('toc');
        _a('#tocCloseBtn', 'title', 'toc_close');
        const tocTitleEl = document.querySelector('#previewToc .preview-toc-title');
        if (tocTitleEl) tocTitleEl.textContent = t('toc');
        const tocEmptyEl = document.querySelector('#previewTocList .preview-toc-empty');
        if (tocEmptyEl) tocEmptyEl.textContent = t('toc_empty');
